...
cl_mem need_to_change_before_buffer;
cl_mem need_to_change_after_buffer;
...
//   *
//   *
  need_to_change_before_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE,  sizeof(char) * GRAIN * GRAIN, NULL, NULL);
  need_to_change_after_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE,  sizeof(char) * GRAIN * GRAIN, NULL, NULL);

  if (!need_to_change_before_buffer || !need_to_change_after_buffer) exit_with_error ("Failed to allocate need_to_change buffer");
  cl_int err = 0;
